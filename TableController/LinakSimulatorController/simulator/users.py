...
class SeatedUser(UserBehavior):
    ...
    def simulate(self, time_delta_s):
        if self.desk.state["position_mm"] > self.preffered_position:
            logging.info(f"SeatedUser adjusting desk {self.desk.desk_id} to seated position {self.prefferred_position}.")
...
class StandingUser(UserBehavior):
    ...
    def simulate(self, time_delta_s):
        if self.desk.state["position_mm"] < self.preffered_position:
            logging.info(f"StandingUser adjusting desk {self.desk.desk_id} to standing position {self.preffered_position}.")
            self.desk.set_target_position(self.preffered_position)
